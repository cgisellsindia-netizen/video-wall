(function () {
  ...
  function buildEmbedUrlNoLoop(source) {
    if (source.type === 'youtube') {
      const youtubeId = parseYouTubeId(source.url);
      if (!youtubeId) return '';
    ...
    if (source.type === 'vimeo') {
      const vimeoId = parseVimeoId(source.url);
      if (!vimeoId) return '';
      return 'https://player.vimeo.com/video/' + vimeoId + '?autoplay=1&muted=1&autopause=0';
    }
    if (source.type === 'page') {
      return source.url;
    }
  ...
  app.innerHTML = [
    ...
    '          </select>',
    '        </label>',
    '        <label class="field" style="display:flex;flex-direction:row;align-items:center;gap:10px;margin-bottom:14px;">',
    '          <span style="margin-bottom:0;">IP rotation</span>',
    '          <input type="checkbox" id="proxy-enabled-input" style="width:20px;height:20px;margin:0;" />',
    '          <span style="font-size:13px;color:var(--muted);font-weight:400;text-transform:none;letter-spacing:0;">Change IP on every replay</span>',
    '        </label>',
  ...
  function createTileController(article, camera) {
    ...
    function playCurrentSource(reason) {
      ...
      if (source.type === 'youtube' || source.type === 'vimeo' || source.type === 'page') {
        const embedUrl = buildEmbedUrlNoLoop(source);
        if (!embedUrl) {
        ...
        scheduleIframeReload(iframe, source, proxyBadgeText);
        setStatus(
          article,
    ...
    function replaySameSourceWithNewProxy() {
      clearRetry();
      destroyMedia();
      playCurrentSource('replay');
    }

    function scheduleIframeReload(iframe, source, proxyBadgeText) {
      var reloadMs = 45000;
      clearRetry();
      retryTimer = window.setTimeout(function () {
        var pUrl = pickNextProxy();
        var newBadge = '';
        if (pUrl) {
          newBadge = 'proxy ' + ((proxyRoundRobin - 1 + proxyPool.length) % proxyPool.length + 1) + '/' + proxyPool.length;
        }
        var embedUrl = buildEmbedUrlNoLoop(source);
        iframe.src = pUrl ? wrapUrlThroughProxy(embedUrl, pUrl) : embedUrl;
        setStatus(article, 'Reloaded with new IP', 'is-live', source.url, newBadge);
        scheduleIframeReload(iframe, source, newBadge);
      }, reloadMs);
    }
